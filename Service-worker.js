// Update cache version when you deploy
const CACHE_NAME = 'agora-precheck-v7';
...
const urlsToCache = [
  ...
  // Logo (safe to cache for offline)
...
// Cache-first for static; network-first for others
self.addEventListener('fetch', (event) => {
  const req = event.request;
  const url = new URL(req.url);

  // Never try to cache Firebase JSON farm data; always network (so we see latest)
